...
function message(el,text,type=''){el.textContent=text;el.className='message'+(type?` ${type}`:'')}
...
async function getSignedUrl(path,expiresIn=3600){
  ...
  const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.message||data.error||'Could not open this song.');
  const signed=data.signedURL||data.signedUrl;if(!signed)throw new Error('No signed media link was returned.');
...
async function loadTracks(silent=false){
  ...
  tracks=await r.json();renderTracks();message(libraryMessage,tracks.length?'Library is up to date.':'No special songs have been published yet.',tracks.length?'success':'');
}

function renderTracks(){
  trackCount.textContent=`${tracks.length} track${tracks.length===1?'':'s'}`;
  trackList.innerHTML=tracks.map((t,i)=>`<div class="track ${t.id===currentId?'active':''}" data-id="${esc(t.id)}"><div class="track-num">${String(i+1).padStart(2,'0')}</div><div><div class="track-title">${esc(t.title)}</div><div class="track-meta">SOXLO Private Release</div></div><button class="play" type="button" data-play="${esc(t.id)}">▶</button><button class="download" type="button" data-download="${esc(t.id)}">Download ↓</button></div>`).join('');
  trackList.querySelectorAll('[data-play]').forEach(btn=>btn.addEventListener('click',()=>playTrack(btn.dataset.play)));
...
async function playTrack(id){
  ...
  try{const url=await getSignedUrl(t.storage_path,3600);currentId=t.id;trackTitle.textContent=t.title;nowTitle.textContent=t.title;audio.src=url;renderTracks();await audio.play()}catch(e){message(libraryMessage,e.message,'error')}
}
...
async function uploadSong(){
  ...
  const upload=await api(`${cfg.url}/storage/v1/object/${BUCKET}/${encodePath(path)}`,{method:'POST',headers:{'Content-Type':file.type||'application/octet-stream','x-upsert':'false'},body:file});
  ...
  uploadForm.reset();message(uploadMessage,'Published. Members and installed players will see it automatically.','success');await loadTracks(true);
}
...
logoutBtn.addEventListener('click',()=>{audio.pause();audio.removeAttribute('src');clearSession();showLogin()});
